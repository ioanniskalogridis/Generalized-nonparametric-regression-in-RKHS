...
// s is a smoothness parameter, s = m+d/2 for the Sobolev space of order $m$ on $R^d$
// ls  a bandwidth parameter, by default equal to 1
...
// K(x,y) = Π_j K_s(x_j, y_j) - product Matérn kernel
...
mat tensor_sobolev_kernel(const mat &X, const mat &Y, double ls, double s)
{
    ...
    for (uword j = 0; j < d; ++j)
    {
        mat Xj = X.col(j);
        mat Yj = Y.col(j);
        mat Kj = matern_kernel(Xj, Yj, s, ls);
        K = K % Kj;
    }
    return K;
}
...
// Quantile IRLS with local quadratic approximation (due to Nyckha et al. (1996))
// tau is the quantile to be estimated, by default equal to 0.5 (median)
// max_iter is the number of maximum iterations to be carried out by IRLS
// eps is the tuning parameter of the local quadratic approximation, see Section 5 of the paper
// tol is the tolerance determining successful convergence
